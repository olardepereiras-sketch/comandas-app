...
export const listSubscriptionPlansProcedure = publicProcedure.query(async ({ ctx }) => {
  console.log('🔵 [LIST SUBSCRIPTION PLANS] Obteniendo planes...');
  const result = await ctx.db.query(
    'SELECT * FROM subscription_plans WHERE is_visible = true ORDER BY price ASC'
  );
  
  console.log('🔍 [LIST SUBSCRIPTION PLANS] Planes encontrados:', result.rows.length);
  ...
  const plans: SubscriptionPlan[] = result.rows.map((row: any) => {
    ...
    try {
      ...
      console.warn('⚠️ [LIST SUBSCRIPTION PLANS] Failed to parse enabled_modules for plan:', row.id, err);
    }
    
    console.log('📋 [LIST SUBSCRIPTION PLANS] Plan:', {
      id: row.id,
      name: row.name,
    ...
    try {
      ...
      console.warn('⚠️ [LIST SUBSCRIPTION PLANS] Failed to parse allowed_duration_ids for plan:', row.id, err);
    }
  ...
  console.log('✅ [LIST SUBSCRIPTION PLANS] Total planes retornados:', plans.length);
  return plans;
});
