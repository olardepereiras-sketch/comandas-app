...
export const getReservationByTokenProcedure = publicProcedure
  .input(
    z.object({
      token: z.string(),
    })
  ...
  .query(async ({ input, ctx }) => {
    console.log('🔍 [GET RESERVATION BY TOKEN] Buscando reserva con token:', input.token);

    let reservation = await ctx.db.query(
      `SELECT r.*, 
       ...
       WHERE r.confirmation_token = $1 AND r.status != 'modified'`,
      [input.token]
    );

    if (reservation.rows.length === 0) {
      console.log('⚠️ [GET RESERVATION BY TOKEN] No encontrada por confirmation_token, intentando con token...');
      reservation = await ctx.db.query(
        `SELECT r.*, 
                c.name as client_name, 
                c.phone as client_phone,
                c.rating as client_rating,
         ...
         JOIN restaurants rest ON r.restaurant_id = rest.id
         LEFT JOIN table_locations l ON r.location_id = l.id
         WHERE r.token = $1 AND r.status != 'modified'`,
        [input.token]
      );
    }
    ...
    console.log('✅ [GET RESERVATION BY TOKEN] Reserva encontrada:', row.id);
    ...
    return {
      ...
      restaurantGoogleMaps: row.google_maps_url || null,
      ...
      guests: row.guests,
      status: row.status,
      notes: row.status === 'cancelled' ? null : row.notes,
