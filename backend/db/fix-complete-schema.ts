...
async function fixCompleteSchema() {
  ...
  try {
    ...
    // 1. Arreglar rating_criteria
    console.log('\n📋 1. Arreglando tabla rating_criteria...');
    const ratingCriteriaCheck = await client.query(`
      SELECT column_name 
      FROM information_schema.columns 
      WHERE table_name = 'rating_criteria'
    `);
    
    const hasUpdatedAt = ratingCriteriaCheck.rows.some(r => r.column_name === 'updated_at');
    ...
    // 2. Arreglar day_exceptions
    console.log('\n📅 2. Arreglando tabla day_exceptions...');
    ...
    console.log('\n✅ Esquema verificado y corregido exitosamente');
  } catch (error) {
...
fixCompleteSchema().catch(console.error);
