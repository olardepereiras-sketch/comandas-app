...
export default function WaiterScreen() {
  ...
  useEffect(() => {
    ...
    vanillaClient.floorPlan.get.query({ restaurantId, locationId: selectedLocationId })
      .then(res => {
        if (res?.planData && Array.isArray(res.planData)) {
          setFloorPlanItems(res.planData);
          console.log('[Waiter] FloorPlan loaded:', res.planData.length, 'items');
        }
      })
      .catch(e => console.log('[Waiter] FloorPlan load error:', e));
  ...
  const getTablePos = useCallback((tableId: string, index: number): TablePosition => {
    ...
    return getDefaultPos(index, canvasSize.h);
  }, [positions, canvasSize.h]);
          ...
          >
            {canvasSize.w > 0 && canvasSize.h > 0 && (
              <View style={{
                ...
                width: canvasSize.h,
                height: canvasSize.w,
                top: (canvasSize.h - canvasSize.w) / 2,
                left: (canvasSize.w - canvasSize.h) / 2,
                transform: [{ rotate: '90deg' }],
                overflow: 'hidden',
                backgroundColor: '#eef0f3',
              }}>
                <View style={{ width: canvasSize.h, height: canvasSize.w, position: 'relative' }}>
                  {currentTables.length === 0 ? (
                    ...
                    currentTables.map((table, index) => {
                      ...
                      const borderColor = hasReady ? '#86efac' : hasOrder ? '#fed7aa' : '#cbd5e1';

                      const planItem = floorPlanItems.find(p => p.tableId === table.id);
                      const pos = planItem
                        ? { x: planItem.x, y: planItem.y, rotation: planItem.rotation }
                        : getTablePos(table.id, index);
                      const tableW = planItem ? planItem.width : CARD_W;
                      const tableH = planItem ? planItem.height : CARD_H;
                      const shape = planItem?.shape || 'rectangle';
                      const isCircle = shape === 'circle';
                      const chairSize = 10;
                      const chairGap = 4;
                      const chairColor = hasOrder ? (hasReady ? '#86efac' : '#fed7aa') : '#94a3b8';

                      const chairsTop: number[] = [];
                      const chairsBottom: number[] = [];
                      const chairsLeft: number[] = [];
                      const chairsRight: number[] = [];

                      if (!isCircle) {
                        const hChairs = Math.max(1, Math.floor(tableW / (chairSize + chairGap + 4)));
                        const vChairs = Math.max(1, Math.floor(tableH / (chairSize + chairGap + 4)));
                        const hStep = tableW / (hChairs + 1);
                        const vStep = tableH / (vChairs + 1);
                        for (let c = 1; c <= hChairs; c++) {
                          chairsTop.push(hStep * c - chairSize / 2);
                          chairsBottom.push(hStep * c - chairSize / 2);
                        }
                        for (let c = 1; c <= vChairs; c++) {
                          chairsLeft.push(vStep * c - chairSize / 2);
                          chairsRight.push(vStep * c - chairSize / 2);
                        }
                      }

                      const numCircleChairs = isCircle ? Math.min(table.capacity || 4, 8) : 0;
                      const radius = isCircle ? Math.min(tableW, tableH) / 2 : 0;
                      const chairRadius = radius + chairSize / 2 + chairGap;
                      ...
                      return (
                        <View
                          ...
                          style={{
                            position: 'absolute',
                            left: pos.x - chairGap - (isCircle ? 0 : 0),
                            top: pos.y - chairGap - (isCircle ? 0 : 0),
                            width: tableW + (isCircle ? 0 : 0),
                            height: tableH + (isCircle ? 0 : 0),
                          }}
                        >
                          {!isCircle && chairsTop.map((cx, i) => (
                            <View key={`ct${i}`} style={[ss.chair, { backgroundColor: chairColor, width: chairSize, height: chairSize, left: cx, top: -(chairSize + chairGap) }]} />
                          ))}
                          {!isCircle && chairsBottom.map((cx, i) => (
                            <View key={`cb${i}`} style={[ss.chair, { backgroundColor: chairColor, width: chairSize, height: chairSize, left: cx, top: tableH + chairGap }]} />
                          ))}
                          {!isCircle && chairsLeft.map((cy, i) => (
                            <View key={`cl${i}`} style={[ss.chair, { backgroundColor: chairColor, width: chairSize, height: chairSize, left: -(chairSize + chairGap), top: cy }]} />
                          ))}
                          {!isCircle && chairsRight.map((cy, i) => (
                            <View key={`cr${i}`} style={[ss.chair, { backgroundColor: chairColor, width: chairSize, height: chairSize, left: tableW + chairGap, top: cy }]} />
                          ))}

                          {isCircle && Array.from({ length: numCircleChairs }).map((_, i) => {
                            const angle = (2 * Math.PI * i) / numCircleChairs - Math.PI / 2;
                            const cx = radius + Math.cos(angle) * chairRadius - chairSize / 2;
                            const cy = radius + Math.sin(angle) * chairRadius - chairSize / 2;
                            return (
                              <View key={`cc${i}`} style={[ss.chair, {
                                backgroundColor: chairColor,
                                width: chairSize, height: chairSize,
                                borderRadius: chairSize / 2,
                                left: cx, top: cy,
                              }]} />
                            );
                          })}
                          ...
                          <TouchableOpacity
                            style={[
                              ss.tableCard,
                              {
                                ...
                                borderColor,
                                borderRadius: isCircle ? tableW / 2 : 10,
                            ...
                            activeOpacity={0.75}
                          >
                            <Text style={ss.tableCardName} numberOfLines={1}>{table.name}</Text>
            ...
            {categories.length === 0 ? (
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={ss.catRowContent} bounces={false} style={ss.catSingleRow}>
                {[...categories].sort((a, b) => {
                  ...
                  const foA = famA ? famA.order : 999;
                  const foB = famB ? famB.order : 999;
                  return foA !== foB ? foA - foB : a.order - b.order;
                }).map(cat => {
                  const isActive = selectedCatId === cat.id;
                  const catColor = cat.color || C.accent;
                  const fam = families.find(f => f.id === cat.familyId);
                  const famColor = fam?.color || catColor;
                  return (
                    <TouchableOpacity
                      key={cat.id}
                      style={[
                        ss.catChip,
                        { backgroundColor: catColor, borderLeftColor: famColor, borderLeftWidth: 4 },
                        isActive && ss.catChipActive,
                      ]}
              ...
              <ScrollView style={ss.productsArea} showsVerticalScrollIndicator={false} contentContainerStyle={ss.productsGrid}>
                {currentCatProducts.map(prod => {
                  ...
                  return (
                    <TouchableOpacity
                      key={prod.id}
                      style={[ss.productBtn, outOfStock && ss.productBtnDisabled]}
        ...
        <View style={[ss.bottomBar, { paddingBottom: insets.bottom + 4 }]}>
          <View style={ss.bottomBarContent}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (!activeOrder) return;
                const updatedOrder = { ...activeOrder, isClaimed: true, claimedAt: Date.now() };
                const next = orders.map(o => o.id === updatedOrder.id ? updatedOrder : o);
                setActiveOrder(updatedOrder);
                void saveOrders(next);
                Alert.alert('¡Pedido reclamado!', 'Se ha enviado una alerta urgente al monitor de cocina.');
              }}
              activeOpacity={0.8}
            >
...
const SIDEBAR_W = IS_TABLET ? 90 : 72;
...
const ss = StyleSheet.create({
  ...
  catSection: { backgroundColor: '#1e293b', borderBottomWidth: 1, borderBottomColor: '#334155' },
  catSingleRow: { maxHeight: 62 },
  catRowContent: { paddingHorizontal: 6, paddingVertical: 6, gap: 6, alignItems: 'center' as const },
  catChip: {
  ...
  productBtnName: { fontSize: 11, fontWeight: '700' as const, color: C.text, lineHeight: 15, textAlign: 'center' as const },
