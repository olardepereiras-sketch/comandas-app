...
async function playNativeBeep(vol: number): Promise<void> {
  try {
    ...
    console.log('[Kitchen] Native audio beep playing, vol:', vol);
  } catch (e) {
    console.log('[Kitchen] Native audio error, falling back to vibration:', e);
...
export default function KitchenScreen() {
  ...
  const [pendingItemPrint, setPendingItemPrint] = useState<Set<string>>(new Set());
  const [isAudioLocked, setIsAudioLocked] = useState<boolean>(Platform.OS === 'web');
  const lastKnownRef = useRef<Map<string, { itemCount: number; status: string; isClaimed: boolean }>>(new Map());
  ...
  // Lock orientation to landscape on native
  useEffect(() => {
    if (Platform.OS !== 'web') {
      ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.LANDSCAPE).catch(e =>
        console.log('[Kitchen] Orientation lock error:', e)
      );
      return () => {
  ...
  useEffect(() => {
    ...
    if (isFirstSyncRef.current) {
      lastKnownRef.current = new Map(
        orders.map(o => [o.id, { itemCount: o.items.filter(i => i.status !== 'pending').length, status: o.status, isClaimed: o.isClaimed || false }])
      );
    ...
    const newClaimedIds = new Set<string>();
    orders.forEach(o => {
      ...
      if (!known) {
        ...
        if (o.isClaimed && o.status !== 'closed') {
          console.log('[Kitchen] RECLAMADA (nueva):', o.tableLabel);
          newClaimedIds.add(o.id);
        }
      } else {
        ...
        if (o.isClaimed && !known.isClaimed && o.status !== 'closed') {
          console.log('[Kitchen] RECLAMADA:', o.tableLabel);
          newClaimedIds.add(o.id);
        }
      }
    });
    ...
    if (newClaimedIds.size > 0) {
      console.log('[Kitchen] Reproduciendo alarma para', newClaimedIds.size, 'pedido(s) reclamado(s)');
      setClaimedOrderIds(prev => new Set([...prev, ...newClaimedIds]));
      setTimeout(() => playAlarmSoundRef.current(), 50);
    }
  }, [orders]);
  ...
  // Fix: mark all active items as 'served' so completed orders don't reappear
  const handleOrderServed = useCallback((orderId: string) => {
    const now = Date.now();
    const next = orders.map((o: Order) => o.id === orderId
      ? {
          ...o,
          status: 'closed' as const,
          lastServedAt: now,
          items: o.items.map((i: OrderItem) =>
            (i.status === 'ready' || i.status === 'preparing')
              ? { ...i, status: 'served' as const }
              : i
          ),
        }
      : o
    ...
    setClaimedOrderIds(prev => { const s = new Set(prev); s.delete(orderId); return s; });
  ...
  const handlePrintItems = useCallback(async (orderData: Order, toPrint: OrderItem[]): Promise<void> => {
    ...
    for (const [printerId, items] of Object.entries(byPrinter)) {
      ...
      const itemParams = items.map((i) => ({
        qty: i.qty,
        name: i.name,
        // Fix: don't send notes when selectedCharacteristics exist (prevents double printing)
        notes: (i.selectedCharacteristics && i.selectedCharacteristics.length > 0)
          ? undefined
          : (i.notes || undefined),
  ...
  const handlePrintSingleItem = useCallback(async (orderData: Order, item: OrderItem): Promise<void> => {
    setPendingItemPrint(prev => {
      const s = new Set(prev);
      s.delete(item.id);
      return s;
    });
    setPrintedItemIds(prev => {
      const s = new Set(prev);
      s.add(item.id);
      return s;
    });
    console.log('[Kitchen] Imprimiendo item individual:', item.name);
    await handlePrintItems(orderData, [item]);
  }, [handlePrintItems]);
  ...
  const allTickets = useMemo(() => {
    ...
    orders.forEach((o: Order) => {
      // An order shows only if it has items that are preparing/ready (not served/pending)
      ...
      // Don't show closed orders with only ready items (they were just marked done)
      const hasActivePendingItems = o.items.some(i => i.status === 'preparing' || i.status === 'ready');
          ...
          >
            <View style={[st.columnGrid, availableHeight > 0 ? { height: availableHeight } : {}]}>
              ...
              {viewMode === 'pendientes' && ticketChunks.map(({ order, items, partIndex, totalParts }: { order: Order; items: OrderItem[]; partIndex: number; totalParts: number }) => {
                ...
                const isOrderClaimed = claimedOrderIds.has(order.id) || !!order.isClaimed;
                const activeItems = items.filter((i: OrderItem) => i.status !== 'ready');
                ...
                const accentColor = isOrderClaimed ? '#dc2626' : isNew ? '#f97316' : allDone ? '#16a34a' : hasPreparing ? '#d97706' : '#ea580c';
                const headerBg = isOrderClaimed ? '#fef2f2' : isNew ? '#fff7ed' : allDone ? '#dcfce7' : hasPreparing ? '#fef9c3' : '#fff7ed';
                const ticketKey = `${order.id}-${partIndex}`;
                // Detect repeat items: items added after last served round
                const isRepeatOrder = order.status === 'closed' || (order.lastServedAt && items.some(i => i.createdAt > order.lastServedAt));
                ...
                return (
                  <Animated.View
                    ...
                    style={[
                      ...
                      (isNew || isOrderClaimed) && st.ticketNew,
                      isOrderClaimed && st.ticketClaimed,
                      (isNew || (isOrderClaimed && claimedOrderIds.has(order.id))) && { transform: [{ scale: newPulseAnim }] },
                    ]}
                  >
                    {isOrderClaimed && claimedOrderIds.has(order.id) && !isContinued && (
                      <View style={st.claimedBanner}>
                        <AlertTriangle size={14} color="#b45309" strokeWidth={2.5} fill="#fbbf24" />
                        <Text style={st.claimedBannerText}>¡URGENTE! Pedido reclamado</Text>
                        <TouchableOpacity
                          style={[st.acceptBtn, { backgroundColor: '#dc2626' }]}
                          onPress={() => setClaimedOrderIds(prev => { const s = new Set(prev); s.delete(order.id); return s; })}
                          activeOpacity={0.8}
                        >
                          <Check size={12} color="#fff" strokeWidth={2.5} />
                          <Text style={st.acceptBtnText}>OK</Text>
                        </TouchableOpacity>
                      </View>
                    )}
                    {isNew && !isContinued && (
                    ...
                    {isRepeatOrder && !isContinued && (
                      <View style={st.repeatBanner}>
                        <Text style={st.repeatBannerText}>♻ REPETICIÓN</Text>
                      </View>
                    )}
                    <View style={[st.ticketHeader, { backgroundColor: headerBg }]}>
                      <View style={{ flex: 1 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                          {order.isClaimed && (
                            <AlertTriangle size={15} color="#f59e0b" strokeWidth={2.5} fill="#fbbf24" />
                          )}
                      ...
                      return (
                        <View key={groupKey}>
                          ...
                          {seqItems.map((item) => {
                            ...
                            const isPendingPrint = pendingItemPrint.has(item.id);
                            const isRepeatItem = order.lastServedAt && item.createdAt > order.lastServedAt;
                            if (isReady) {
                              return (
                                <View
                                  key={item.id}
                                  style={[st.itemRow, {
                                    backgroundColor: isPrinted ? '#f0fdf4' : isPendingPrint ? '#eff6ff' : isSelectedPrint ? '#ecfdf5' : '#f8fafc',
                                    borderLeftWidth: isPendingPrint && !isPrinted ? 3 : isSelectedPrint && !isPrinted ? 3 : 0,
                                    borderLeftColor: isPendingPrint ? '#3b82f6' : '#3b82f6',
                                  }]}
                                >
                                  ...
                                  <View style={{ flex: 1 }}>
                                    ...
                                    {item.selectedCharacteristics?.map((c) => (
                                      <Text key={c.charId} style={{ fontSize: 10, color: '#94a3b8' }}>• {c.charName}: {c.optionLabel}</Text>
                                    ))}
                                  </View>
                                  {isPrinted
                                    ? null
                                    : isPendingPrint
                                      ? <TouchableOpacity
                                          style={st.printBtn}
                                          onPress={() => void handlePrintSingleItem(order, item)}
                                          activeOpacity={0.8}
                                        >
                            ...
                            if (isPreparing) {
                              return (
                                <TouchableOpacity
                                  key={item.id}
                                  style={[st.itemRow, {
                                    backgroundColor: isSelectedPrint ? '#eff6ff' : (isRepeatItem ? '#fff8f0' : '#fefce8'),
                                    borderLeftWidth: isSelectedPrint ? 3 : isRepeatItem ? 3 : 0,
                                    borderLeftColor: isRepeatItem ? '#f97316' : '#3b82f6',
                                  }]}
                                ...
                                >
                                  ...
                                  <View style={{ flex: 1 }}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                                      ...
                                      {isRepeatItem && <View style={st.repeatBadge}><Text style={st.repeatBadgeText}>REPETICIÓN</Text></View>}
                                    </View>
                                    {item.notes && !item.selectedCharacteristics?.length ? <Text style={st.itemNotes}>{item.notes}</Text> : null}
                                    {item.selectedCharacteristics?.map((c) => (
                                      <Text key={c.charId} style={{ fontSize: 10, color: '#d97706' }}>• {c.charName}: {c.optionLabel}</Text>
                                    ))}
                                  </View>
                                  {isSelectedPrint
                                    ? <Printer size={13} color="#3b82f6" strokeWidth={2.5} />
                            ...
                            return (
                              <TouchableOpacity
                                ...
                                style={[st.itemRow, {
                                  borderLeftWidth: isSelectedPrint ? 3 : isRepeatItem ? 3 : 0,
                                  borderLeftColor: isRepeatItem ? '#f97316' : '#3b82f6',
                                  backgroundColor: isSelectedPrint ? '#eff6ff' : (isRepeatItem ? '#fff8f0' : '#ffffff'),
                                }]}
                                onPress={() => { if (!isPrinted) togglePrintSelect(item.id); }}
                                activeOpacity={0.7}
                              >
